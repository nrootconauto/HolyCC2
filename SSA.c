...
static graphNodeMapping
filterVarAssigns(graphNodeIR enter, strGraphNodeIRP nodes, struct IRVar *var) {
	// First find the references to the var and jumps.
	struct varAndEnterPair pair;
	pair.enter = enter;
	pair.var = var;
	return createFilteredGraph(
	    enter, nodes, &pair,
...
static void SSAVersionVar(graphNodeIR start, struct IRVar *var) {
	...
	for (long i1 = 0; i1 != strGraphPathSize(allPaths); i1++) {
		//
		//+1 is for final outgoing node
		//
		for (long i2 = 0; i2 != strGraphEdgePSize(allPaths[i1]) + 1; i2++) {
			graphNodeIR node;
			// Use outgoing value on last run
			if (i2 < strGraphEdgePSize(allPaths[i1]))
				node = *graphNodeMappingValuePtr(__graphEdgeIncoming(allPaths[i1][i2]));
			else
				node = *graphNodeMappingValuePtr(
				    __graphEdgeOutgoing(allPaths[i1][i2 - 1]));
	...
	for (;;) {
		...
		for (long i1 = 0; i1 != strGraphPathSize(allPaths); i1++) {
			...
			//
			for (long i2 = 0; i2 != strGraphEdgePSize(allPaths[i1]) + 1; i2++) {
				graphNodeMapping mappedNode;
				// Use outgoing value on last run
				if (i2 < strGraphEdgePSize(allPaths[i1]))
					mappedNode = __graphEdgeIncoming(allPaths[i1][i2]);
				else
					mappedNode = __graphEdgeOutgoing(allPaths[i1][i2 - 1]);

				// Continue if not val
				if (graphNodeIRValuePtr(*graphNodeMappingValuePtr(mappedNode))->type !=
				    IR_VALUE)
					continue;

				__auto_type irVal = (struct IRNodeValue *)graphNodeIRValuePtr(
				    *graphNodeMappingValuePtr(mappedNode));
				// if a var
				if (irVal->val.type != IR_VAL_VAR_REF)
					continue;

				// Choose highest incoming
				__auto_type incoming = graphNodeMappingIncomingNodes(mappedNode);
				...
				pair.enter = NULL;
				pair.var = var;
				if (isAssignedVar(&pair, *graphNodeMappingValuePtr(mappedNode)))
					version++;
...
static strGraphNodeIRP IRSSACompute(graphNodeMapping start, struct IRVar *var) {
	...
	for (__auto_type dom = llDominatorsFirst(doms); dom != NULL;
	     dom = llDominatorsNext(dom)) {
