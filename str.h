...
#define STR_TYPE_FUNCS(type, suffix)                                           \
	...
	inline str##suffix str##suffix##SortedInsert(                                \
	    str##suffix vec, type item, int (*pred)(const type *, const type *)) {   \
		return (str##suffix)__vecSortedInsert(                                     \
		    (struct __vec *)vec, &item, sizeof(item),                              \
	...
	inline type *str##suffix##SortedFind(str##suffix vec, const type data,       \
	                                     int pred(const type *, const type *)) { \
		return __vecSortedFind((struct __vec *)vec, &data, sizeof(type),           \
		                       (int (*)(const void *, const void *))pred);         \
	}                                                                            \
	...
	inline str##suffix str##suffix##SetDifference(                               \
		...
		return (str##suffix)__vecSetDifference(                                    \
		    (struct __vec *)vec, (struct __vec *)vec2, sizeof(type),               \
		    (int (*)(const void *, const void *))pred);                            \
	}                                                                            \
	inline str##suffix str##suffix##RemoveIf(                                    \
	    ...
	    __attribute__((always_inline));                                          \
	inline str##suffix str##suffix##RemoveIf(                                    \
	    str##suffix vec, void const *data,                                       \
	    int pred(const void *, const type *)) {                                  \
		return (str##suffix)__vecRemoveIf(                                         \
		    (struct __vec *)vec, sizeof(type),                                     \
	...
	inline str##suffix str##suffix##Unique(                                      \
	    str##suffix str, int (*pred)(const type *, const type *),                \
	    void (*kill)(void *)) {                                                  \
		return (str##suffix)__vecUnique((struct __vec *)str, sizeof(type),         \
		                                (int (*)(const void *, const void *))pred, \
		                                kill);                                     \
	}                                                                            \
	...
	inline str##suffix str##suffix##SetIntersection(                             \
		...
		return (str##suffix)__vecSetIntersection(                                  \
		    (struct __vec *)a, (struct __vec *)b, sizeof(type),                    \
		    (int (*)(const void *, const void *))pred, kill);                      \
	}                                                                            \
