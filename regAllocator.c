...
static const char *IR_ATTR_NODE_SPILL_LOADS_COMPUTED =
    "COMPUTED_SPILLS_AND_LOADS";
...
static void transparentKill(graphNodeIR node, int preserveEdgeValue) {
	...
	for (long i1 = 0; i1 != strGraphEdgeIRPSize(incoming); i1++)
		for (long i2 = 0; i2 != strGraphEdgeIRPSize(outgoing); i2++)
			graphNodeIRConnect(
			    graphEdgeIRIncoming(incoming[i1]), graphEdgeIROutgoing(outgoing[i2]),
			    (preserveEdgeValue) ? *graphEdgeIRValuePtr(incoming[i1])
			                        : IR_CONN_FLOW);
...
void IRCoalesce(strGraphNodeIRP nodes, graphNodeIR start) {
			...
			findVarLoop:;
				...
				for (long i2 = 0; i2 != strVarRefsSize(refs); i2++) {
					for (long i3 = 0; i3 != strGraphNodeIRPSize(refs[i2]); i3++) {
						...
						__auto_type b =
						    (struct IRNodeValue *)graphNodeIRValuePtr(refs[i2][i3]);
	...
	for (long i = 0; i != strGraphNodeIRPSize(nodes); i++) {
		...
		strGraphEdgeIRP incoming CLEANUP(strGraphEdgeIRPDestroy) =
		    graphNodeIRIncoming(nodes[i]);
		strGraphEdgeIRP filtered CLEANUP(strGraphEdgeIRPDestroy) =
		...
		if (strGraphEdgeIRPSize(filtered) == 1) {
			...
			if (inNode->type == IR_VALUE) {
				...
				if (inValueNode->val.type == IR_VAL_VAR_REF) {
					...
					refs[aIndex] = strGraphNodeIRPSetUnion(refs[aIndex], refs[bIndex],
					                                       (gnCmpType)ptrPtrCmp);

					// Remove bIndex
					memmove(&refs[bIndex], &refs[bIndex + 1],
					        (strVarRefsSize(refs) - bIndex - 1) * sizeof(*refs));
...
static int isFloating(struct object *obj) { return obj == &typeF64; }
...
static int conflictPairCmp(const struct conflictPair *a,
                           const struct conflictPair *b) {
	...
	for (long i = 0; i != strGraphNodeIRLivePSize(allNodes); i++) {
		...
		for (long i2 = 0; i2 != strGraphNodeIRLivePSize(outgoingNodes); i2++) {
			...
			if (regSliceConflict(&masterSlice, find)) {
				...
				if (NULL !=
				    strConflictPairSortedFind(conflicts, backwards, conflictPairCmp))
					continue;

				// Check if exists,if so dont insert
				if (NULL ==
				    strConflictPairSortedFind(conflicts, pair, conflictPairCmp)) {
					double aWeight =
					    interfereMetric(1.1, allNodes[i]); // TODO implememnt cost
...
strInt getColorList(llVertexColor vertexColors) {
	strInt retVal = NULL;
	for (__auto_type node = llVertexColorFirst(vertexColors); node != NULL;
	     node = llVertexColorNext(node)) {
		// Insert into retVal if color isnt already in retVal
		if (NULL ==
		    strIntSortedFind(retVal, llVertexColorValuePtr(node)->color, intCmp))
			retVal = strIntSortedInsert(retVal, llVertexColorValuePtr(node)->color,
			                            intCmp);
	}
...
static int isVarsChooseNode(const struct __graphNode *node,
                            const struct interferencePair *pair) {
	...
	if (value->type == IR_CHOOSE) {
		...
		for (long i2 = 0; i2 != strGraphNodeIRPSize(choose->canidates); i2++) {
			...
			struct IRNodeValue *val =
			    (void *)graphNodeIRValuePtr(choose->canidates[i2]);
...
static int spillOrStoreAt(const struct __graphNode *node,
                          const struct interferencePair *pair) {
	// Check if hit a choose node that "consumes" pair->var. Chooses mark the end
	// of one version of a var and a transition to the next version
	struct IRNode *value = graphNodeIRValuePtr((graphNodeIR)node);
...
static int varToLiveNodeCompare(const struct varToLiveNode *a,
                                const struct varToLiveNode *b) {
...
loop:
	...
	for (long i = 0; i != strGraphNodeIRPSize(allNodes); i++) {
		// Dont revisit
		toRemove = strGraphNodeIRPSortedInsert(toRemove, allNodes[i],
		                                       (gnCmpType)ptrPtrCmp);
		...
		if (strGraphEdgeIRPSize(incoming) == 1) {
			if (*graphEdgeIRValuePtr(incoming[0]) == IR_CONN_DEST) {
				...
				if (isVar(allNodes[i])) {
					...
					if (NULL ==
					    strIRVarSortedFind(liveVars, &value->val.value.var, IRVarCmp2)) {
						...
						if (IRIsDeadExpression(in)) {
							...
							toRemove = strGraphNodeIRPSetUnion(toRemove, removed,
							                                   (gnCmpType)ptrPtrCmp);
...
typedef struct regSlice (*color2RegPredicate)(strRegSlice adjacent,
                                              strRegP avail,
                                              graphNodeIRLive live, int color,
                                              const void *data, long colorCount,
                                              const int *colors);
static struct regSlice color2Reg(strRegSlice adjacent, strRegP avail,
                                 graphNodeIRLive live, int color,
                                 const void *data, long colorCount,
                                 const int *colors) {
	...
	for (long i = 0; i != strRegSliceSize(adjacent); i++) {
		adjRegs =
		    strRegPSortedInsert(adjRegs, adjacent[i].reg, (regCmpType)ptrPtrCmp);
	}
	...
	int *find = bsearch(&color, colors, colorCount, sizeof(int),
...
static struct conflictPair *
__conflictPairFindAffects(graphNodeIRLive node,
                          const struct conflictPair *start,
                          const struct conflictPair *end) {
	for (; start != end; start++) {
		if (start->a == node)
			return (void *)start;
...
strConflictPair conflictPairFindAffects(graphNodeIRLive node,
                                        strConflictPair pairs) {
...
static int conflictPairContains(graphNodeIRLive node,
                                const struct conflictPair *pair) {
...
static void replaceVarsWithSpillOrLoad(strGraphNodeIRLiveP spillNodes,
                                       graphNodeIR enter) {
	...
	for (long i = 0; i != strGraphNodeIRLivePSize(spillNodes); i++) {
		...
		varToLive =
		    strVarToLiveNodeSortedInsert(varToLive, pair, varToLiveNodeCompare);
	}
...
static struct IRVar *getVar(graphNodeIR node) {
	...
	if (irNode->type == IR_SPILL) {
		...
		if (value->val.type == IR_VAL_VAR_REF) {
			...
			__auto_type find =
			    llIRAttrFind(irNode->attrs, IR_ATTR_VARIABLE, IRAttrGetPred);
			if (!find)
				return NULL;
...
static graphNodeIR findSinglePreviousAssign(graphNodeIR node) {
	...
	// multiple entrances(This means there may be multiple values of var so we
	// cant ensure a single one exists)
	//
	for (;;) {
		// Is multiple entry points?
		strGraphEdgeIRP incoming CLEANUP(strGraphEdgeIRPDestroy) =
		    graphNodeIRIncoming(node);
		strGraphEdgeIRP incomingFlow CLEANUP(strGraphEdgeIRPDestroy) =
		    IRGetConnsOfType(incoming, IR_CONN_FLOW);
...
static graphNodeIR rematRecur(graphNodeIR node, strGraphNodeIRP *retmatNodes,
	...
	for (long i = 0; i != strGraphNodeIRPSize(tops2); i++) {
		...
		strRegSlice dummy CLEANUP(strRegSliceDestroy) =
		    strRegSliceAppendItem(NULL, val->val.value.reg);
		if (registerConflict(dummy, affected)) {
			//
			// If register conflict check if said register's value can be
			// rematerialized
			//
			...
			if (assign) {
				...
				__auto_type upper = rematRecur(assign, retmatNodes, operationNodes,
				                               tempVarNodes, affected, top);
				if (upper) {
					// Can be rematerialized
					*tempVarNodes = strGraphNodeIRPSortedInsert(*tempVarNodes, upper,
					                                            (gnCmpType)ptrPtrCmp);
					continue;
				}
			}
...
loop:
	allNodes2 =
	    strGraphNodeIRPSetDifference(allNodes2, visited, (gnCmpType)ptrPtrCmp);
	visited = NULL;
	for (long i = 0; i != strGraphNodeIRPSize(allNodes2); i++) {
		visited = strGraphNodeIRPSortedInsert(visited, allNodes2[i],
		                                      (gnCmpType)ptrPtrCmp);
	...
	for (long i = 0; i != strGraphNodeIRLivePSize(intInterfere); i++) {
		...
		qsort(conflictsSortedByWeight, strConflictPairSize(conflicts),
		      sizeof(*conflicts), conflictPairWeightCmp);
		...
		for (; 0 != strConflictPairSize(conflicts);) {
			...
			__auto_type conflictsWithFirst =
			    conflictPairFindAffects(conflicts[0].a, conflicts);
			...
			for (;;) {
				...
				for (long i = 0; i != strConflictPairSize(clone); i++) {
					conflictsWithFirst = strConflictPairSetUnion(
					    conflictPairFindAffects(clone[i].a, conflicts),
					    conflictsWithFirst, conflictPairCmp);
					conflictsWithFirst = strConflictPairSetUnion(
					    conflictPairFindAffects(clone[i].b, conflicts),
					    conflictsWithFirst, conflictPairCmp);
				}
			...
			for (long i = 0; i != strConflictPairSize(conflictsWithFirst); i++) {
				for (long i2 = 0; i2 != strConflictPairSize(conflictsSortedByWeight);
				     i2++)
		...
		for (long i = 0; i != strGraphNodeIRLivePSize(allColorNodes); i++) {
			if (NULL == strIRVarSortedFind(
			                liveVars, &graphNodeIRLiveValuePtr(allColorNodes[i])->ref,
			                IRVarCmp2))
				liveVars = strIRVarSortedInsert(
				    liveVars, &graphNodeIRLiveValuePtr(allColorNodes[i])->ref,
				    IRVarCmp2);
		}
