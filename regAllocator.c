...
static int noIncomingPred(const void *data,const graphNodeIR *node) {
		strGraphEdgeIRP incoming __attribute__((cleanup(strGraphEdgePDestroy)))=graphNodeIRIncoming(*node);
...
static void replaceNodeWithExpr(graphNodeIR node,graphNodeIR valueSink) {		
		...
		for(long i1=0;i1!=strGraphEdgeIRPSize(outgoing);i1++)
				graphNodeIRConnect(valueSink ,graphEdgeIRIncoming(outgoing[i1]), *graphEdgeIRValuePtr(outgoing[i1]));

		strGraphEdgeIRPDestroy(&incoming),strGraphEdgeIRPDestroy(&outgoing);
}
static void removeChooseNode(graphNodeIR chooseNode) {
		//Kill node,remove outgoing nodes too if they variable they point to is unused
...
static int varRefsInsertCmp(const strGraphNodeIRP *a,const strGraphNodeIRP *b) {
		struct IRNodeValue *valA=(void*)a[0],*valB=(void*)b[0];
		return IRVarCmp((const struct IRVar*)&valA->val.value.var, (const struct IRVar*)&valB->val.value.var);
}
...
void IRCoalesce(strGraphNodeIRP nodes,graphNodeIR start) {
		...
		for(long i=0;i!=strGraphNodeIRPSize(nodes);i++) {
				...
				if(val->type==IR_CHOOSE) {
						...
						//
						// Remove choose nodes whoose incoming vars are all the same ,ALSO,be sure to alias incoming var to choose with outgoing to choose
						//
						strGraphNodeIRP clone __attribute__((cleanup(strGraphNodeIRPDestroy)))=strGraphNodeIRPUnique(strGraphNodeIRPClone(choose->canidates),  (gnCmpType)ptrPtrCmp,  NULL) ;
						if(strGraphNodeIRPSize(clone)==1) {
								...
								strGraphEdgeIRP outgoing __attribute__((cleanup(strGraphEdgeIRPDestroy)))=graphNodeIROutgoing(clone[i]);
								strGraphEdgeIRP outgoingAssigns __attribute__((cleanup(strGraphEdgeIRPDestroy)))=IRGetConnsOfType(outgoing, IR_CONN_DEST);
								...
								for(long i=0;i!=strGraphEdgeIRPSize(outgoingAssigns);i++) {
										...
										if(NULL==strAliasPairSortedFind(aliases, apForward, aliasPairCmp)) {
												aliases=strAliasPairSortedInsert(aliases, apForward, aliasPairCmp);
										}
								}
						}
						...
						findVarLoop:;
								...
								if(!find) {
										__auto_type  newRef =llVarRefsCreate(strGraphNodeIRPAppendItem(NULL, nodes[i]));
								...
								//If one filtered that is a value and a variable,set that as the alias node
		...
		//
		// Merge aliases that share same head/tail untill you cant do it anymore. This allows for multiple vars to be merges
		// For Example:
		// a=b=c
		// would be(before)
		// b<->c
